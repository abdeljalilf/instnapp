...
const RapportFinal = () => {
        ...
        samples.forEach((sample) => {
            ...
            const isDuplicate = acc[sample.sampleReference].analyses.some(
                (a) =>
                    ...
                    a.Valeur_Norme_Utlise === sample.Valeur_Norme_Utlise &&
                    a.Limite_Detection === sample.Limite_Detection &&
                    a.Observation === sample.Observation &&
                    a.Used_norme === sample.Used_norme
            );
    ...
    return (
        <div className="rapport-final-container">
            ...
            {groupedAnalysesArray.map(
                ([sampleReference, { sampleType, samplingLocation, samplingDate, sampledBy, analyses }], index) => {
                    // Déterminer la norme utilisée pour cet échantillon
                    const usedNorme = analyses.length > 0 ? analyses[0].Used_norme : 'Norme Utilisée';
                    ...
                    return (
                        <div key={index} className="sample-table">
                            ...
                            <p>
                                <strong>Lieu de Prélèvement :</strong> {samplingLocation}
                            </p>
                            <p>
                                <strong>Date de Prélèvement :</strong>{' '}
                                {new Date(samplingDate).toLocaleDateString('fr-FR', {
                                    year: 'numeric',
                                    month: 'long',
                                    day: 'numeric',
                                })}
                            ...
                            <p>
                                <strong>Prélevé par :</strong> {sampledBy}
                            </p>
                            <table>
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    {analyses.map((sample, idx) => (
                                        <tr key={idx}>
                                            ...
                                            <td>{sample.Valeur_Norme_Utlise}</td>
